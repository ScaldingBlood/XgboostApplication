...
def reshape(pred):
    ...
    result = result[result['rank']<3][['orderid','geohashed_end_loc','rank']] # [orderid, geohashed_end_loc, rank]
    result = result.set_index(['orderid','rank']).unstack() # [geohashed_end_loc, rank]
...
def get_label(data):
    ...
    if os.path.exists(result_path):
        ...
        true = dict(zip(data_tmp['orderid'].values, data_tmp['geohashed_end_loc']))
        pickle.dump(true, open(result_path, 'wb+'))
